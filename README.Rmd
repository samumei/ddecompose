...
where $\beta_{g,0}$ is the intercept and $\beta_{g,k}$ are the slope coefficents of
...
which accounts for mean differencs of the covariates. Note
...
terms. Since the method hinges on a additive linear model, the terms can be further
decomposed into the contribution of each covariate $k$ in a \emph{detailed
...
a counterfactual outcome distributions based on the conditional outcome distribution
of group and the covariates distribution of the other group. For instance, we 
would reweight the outcome distribution of group 0 such that its covariates
distribution matches the one of group 1.
...
where $P(g)$ and $P(g|x)$ correspond to the (conditional) probabilities of
belonging to group $g$. We can estimate the reweighting factor with sample 
...
conditional outcome distribution as long as we can assum common support and ignorability.
The latter states that the distribution of unobserved covariates conditional 
on observed covariates is independent on the group.
...
(e.g. union status) from the one of covariate $X_2$ (e.g. industry).
