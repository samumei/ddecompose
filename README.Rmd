...
which accounts for mean differencs attributed to different covariate means. Note
...
The Oaxaca-Blinder decomposition is limited to the mean. Moreover, it estimates 
biased decomposition terms if the conditional expectation of outcome given 
covariates is not linear. An alternative is the approach of DiNardo, Fortin, 
and Lemieux (1996) that uses inverse probability to derive an entire 
counterfactual wage distribution of group 0 if it had the same covariates
distribution like group 1. 

Definition of counterfactual

Definition of reweighting factor

Assumption: common support, conditional idependence (CIA)

Limits: Only aggregate decomposition

### Sequential decomposition (DFL)

Limits: path-dependence
