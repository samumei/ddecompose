...
In the following, we decompose again the changes in the US male wage distribution
...
As before, we use again the observations from the early 1980s as reference group. 
```{r eval = TRUE}
...
experience (“Comp. eff. X1|X2”) was negative for all but the two highest deciles.

```{r eval = TRUE}
plot(male_inequality_sequential)
```
In the example above, we first reweighted the distribution of the early 1980s such
that its human capital endowment matches the one of the early 2000s. Thus, the 
counterfactual identifying the effect of unionization combines the human capital 
...
with the left-most conditional probability model in the formula model instead of 
the right-most. 
```{r eval = TRUE}
...
with the (conditional) unionization at deciles.
```{r eval = TRUE}
