...
nlys00$industry <- factor(nlys00$industry, levels = c(
...
nlys00$education <- factor(nlys00$education, levels = c(
...
nlys00 %>%
  group_by(education) %>%
  dplyr::summarise(
    school = length(which(sch10_12 == 1)),
...
nlys00 %>%
  group_by(region) %>%
  dplyr::summarise(n = length(which(north_central == 1)), s = length(which(south00 == 1)), w = length(which(west == 1)))
...
# Variable selection
sel_var_org <- c(
  "lropc00", "female", "age00", "msa", "ctrlcity", "north_central",
...
sel_var <- c(
  "female", "lwage", "wage", "age", "central_city", "msa", "region", "black",
...
round(cbind(
  apply(model.matrix(mod1, subset(nlys00, female != "yes")), 2, mean),
  apply(model.matrix(mod1, subset(nlys00, female == "yes")), 2, mean)
), 3)

nlys00 %>%
  group_by(female) %>%
  dplyr::summarise(mean(lwage)) %>%
  as.data.frame()
...
sel_var <- c(
  "female", "wage", "age", "central_city", "msa", "region", "black",
  "hispanic", "education", "afqt", "family_responsibility",
  "years_worked_civilian", "years_worked_military", "part_time", "industry"
)
nlys00 <- nlys00[, sel_var]
