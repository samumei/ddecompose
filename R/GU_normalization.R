# Peform normalization by Gardeazabal/Ugidos (2004) if requested 
...
GU_normalization <- function(formula, data, weights, group){
  ...
  #data_used$drop.unused.levels = TRUE
  ...
  if(attr(function_terms, "intercept")==1){
    ...
    regressors_for_prediction <- cbind(rep(1, nrow(unadjusted_regressors)), regressors_for_prediction)
