...
<span style="color:red"> The original decomposition method introduced by
Oaxaca (1973) and Blinder (1973) decomposes the difference in the mean
of an outcome variable (e.g. wage) between two groups $g = A, B$ into a
part explained by differences in covariate means (e.g.  educational
level or experience) and another part due different linear regression
...
between the outcome $Y$ and covariates $X$, i.e. 
$$Y_{g,i} = \beta_{g,0} + \sum^K_{k=1}X_{k,i}\beta_{g,k} + \varepsilon_{g,i},$$
where $\beta_{g,0}$ and $\beta_{g,k}$ and the intercept and slope
coefficents of covariates $k = 1,..., K$, respectively. Moreover, it is
assumed that the error term $\varepsilon$ is conditionally independent
of $X$, i.e.  $E( \varepsilon_{g,i} | X_{1,i} \ldots X_{k,i}) = 0$, and
that there is an overlap in observable characteristics across groups
(‘common support’). </span>

<span style="color:red"> Estimating the coefficients with OLS, one can
fit a counterfactual mean  
$$\overline Y_C = \beta_{A,0} + \sum^K_{k=1}\overline X_{B,k} \widehat \beta_{A,k}$$
that would be observed if group $B$ had the same coefficients as group
$A$. By adding and substracting the counterfactual, the observed
...
that captures outcome differences due to differences in coefficients,
and the composition effect
$$\widehat\Delta^\mu_X = \sum^K_{k=1} (\overline X_{Bk} - \overline X_{Ak})\widehat \beta_{Ak}. $$
which identifies mean differencs attributed to different covariate
means. Note we could also combine coefficients from group B with
covariates of group A to perform the decomposition, instead. Such a
change in the ‘reference’ group generally leads to different results.
</span>
