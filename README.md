...
matters if we reweight union coverage by industry ($F(x_1 \mid x_2)$) or
the industry employment given union coverage ($F(x_2 \mid x_1)$).
...
$\widehat \beta_{C,k}$, the true underlying structure effect can be
...
of covariates if the (wage) structure remains the same. Thereby, the
